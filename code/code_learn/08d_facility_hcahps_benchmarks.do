...
* PURPOSE: Facility-Level HCAHPS Benchmarks (Outcomes, Behaviors, Labor)
* FEATURES: Market-Proxy Merging, Clean Titles, Delta Analyses, Memory Safe
*===============================================================================
...
display "1. Aggregating Provider Behaviors & Labor Supply to Facility Level..."
...
bysort ccn: egen pre_h100_1519 = mean(cond(year>=2013 & year<=2014, hcahps_100_score, .))
...
* --- 6. GRAPHING ENGINE ---
display "5. Generating Journal-Ready Visualizations with Deltas..."
local baseOut "$outRoot/summary_stats/in_patient/benchmarks_hcahps_facility"
...
* Re-sort the panel chronologically after bysort commands scrambled it
sort panel_id year
    ...
    if _rc == 0 {
        ...
        * --- TITLE DICTIONARY ---
        if "`var'" == "partd_opioid_rate" {
            ...
            local v_note "Outcome: Readmission Reduction Program Ratio for Heart Attacks."
        }
        else if "`var'" == "rrp_excess_ratio_hf" {
            local c_title "Heart Failure Readmission Ratio"
            local v_note "Outcome: Readmission Reduction Program Ratio for Heart Failure."
        }
        else if "`var'" == "rrp_excess_ratio_pn" {
            local c_title "Pneumonia Readmission Ratio"
            local v_note "Outcome: Readmission Reduction Program Ratio for Pneumonia."
        }
            ...
            local v_note "Outcome: MSPB Ratio tracking cost efficiency."
        }
        ...
        * --- HCAHPS METRICS LOOP ---
        foreach m in `m_list' {
            ...
            local ft_dec "Decline: Binary (1=Yes) if the facility's `m_title' score dropped from two years prior to one year prior."
        ...
        * MEMORY FIX: Flush Stata's graph memory
...
foreach l in `lab_vars' {
    ...
    local ft_pre1 "Benchmark: SD from National mean during pre-period (2013-2014)."
    local ft_pre2 "Benchmark: SD from National mean during pre-period (2013-2020)."

    capture binscatter d_`l'_1519 bin_pre_1519 if year == 2019, discrete line(connect) title("`l_upper' Shift (2015-2019) by Pre-Sample", size(msmall)) xtitle("SDs from Nat. Mean (2013-2014 Avg)") ytitle("`c_title'") note("Outcome: Absolute change in `l_upper' proportion (2015-2019)." "`ft_pre1'", size(vsmall)) graphregion(color(white))
    capture graph export "`baseOut'/universal/longdiff_2015_2019_`l'.png", replace width(2000)
    
    capture binscatter d_`l'_2023 bin_pre_2023 if year == 2023, discrete line(connect) title("`l_upper' Shift (2020-2023) by Pre-Sample", size(msmall)) xtitle("SDs from Nat. Mean (2013-2020 Avg)") ytitle("`c_title'") note("Outcome: Absolute change in `l_upper' proportion (2020-2023)." "`ft_pre2'", size(vsmall)) graphregion(color(white))
    ...
    foreach a in `auths' {
        ...
        capture binscatter d_`l'_1519 bin_pre_1519 if year == 2019 & np_auth == "`a'", discrete line(connect) title("`l_upper' Shift (2015-2019): `a_clean'", size(msmall)) xtitle("SDs from Nat. Mean (2013-2014 Avg)") ytitle("`c_title'") graphregion(color(white))
        capture graph export "`baseOut'/by_np_auth/`a'/longdiff_2015_2019_`l'.png", replace width(2000)
        
        capture binscatter d_`l'_2023 bin_pre_2023 if year == 2023 & np_auth == "`a'", discrete line(connect) title("`l_upper' Shift (2020-2023): `a_clean'", size(msmall)) xtitle("SDs from Nat. Mean (2013-2020 Avg)") ytitle("`c_title'") graphregion(color(white))
    ...
    foreach o in `owns' {
        ...
        capture binscatter d_`l'_1519 bin_pre_1519 if year == 2019 & own_cat == "`o'", discrete line(connect) title("`l_upper' Shift (2015-2019): `o_clean'", size(msmall)) xtitle("SDs from Nat. Mean (2013-2014 Avg)") ytitle("`c_title'") graphregion(color(white))
        capture graph export "`baseOut'/by_ownership/`o'/longdiff_2015_2019_`l'.png", replace width(2000)
        
        capture binscatter d_`l'_2023 bin_pre_2023 if year == 2023 & own_cat == "`o'", discrete line(connect) title("`l_upper' Shift (2020-2023): `o_clean'", size(msmall)) xtitle("SDs from Nat. Mean (2013-2020 Avg)") ytitle("`c_title'") graphregion(color(white))
