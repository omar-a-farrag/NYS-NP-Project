...
* PURPOSE: Binscatters mapping Facility Behavior vs HCAHPS/OAS CAHPS
* FEATURES: Testing Toggle, Both Directions, Extensive Subgroups, Full Dictionary
*===============================================================================
set more off, permanently
include "C:/Users/omarf/Dropbox/personal_files_omar_farrag/Research/projects/NYS_npPolicy/code/00_initialize.do"

* --- START LOG ---
local script_name "08b_facility_quality_curves_test"
...
foreach setting in "in_patient" "out_patient" {
    display "=== STARTING QUALITY CURVES FOR: `setting' FACILITIES ==="
    ...
    * --- 2. LOAD DATA & PREP PANEL ---
    if "`setting'" == "in_patient" {
        ...
        * Handle differing ID structures for ASC vs HOPD
    ...
    * If state_str looks like numeric codes rather than abbreviations, re-try with labels
    capture confirm string variable state_str
    if _rc == 0 {
        quietly count if regexm(state_str, "^[A-Za-z]")
        if r(N) == 0 {
            capture drop state_str
            capture decode `st_var', gen(state_str)
        }
