...
* --- START LOG ---
local script_name "01_macro_inpatient_stats"
...
local outDir "$outRoot/summary_stats/in_patient/macro_stats"
...
foreach var in `all_vars' {
    ...
    *--------------------------------------------------
    ...
    local own_note "Ownership: Gov (Fed/State/Local), For-Profit (Proprietary), Non-Profit (Private)."
    ...
    *--------------------------------------------------
    capture graph bar (mean) `var', over(own_category, label(angle(0) labsize(small))) ///
        ...
        note("`dir_note'" "`np_note'" "`own_note'", size(vsmall)) blabel(bar, format(%9.3f) size(vsmall)) graphregion(color(white) margin(vsmall))
