...
* FEATURES: MIPS Sub-Categories, Clean Titles, Delta Analyses, Memory Safe
*===============================================================================
...
* --- 6. GRAPHING ENGINE ---
display "5. Generating Journal-Ready MIPS Facility Visualizations with Deltas..."
local baseOut "$outRoot/summary_stats/in_patient/benchmarks_mips_facility"
...
foreach l in `lab_vars' {
    ...
    foreach o in `owns' {
        ...
        capture binscatter d_`l'_1719 bin_pre_1719 if year == 2019 & own_cat == "`o'", discrete line(connect) title("`l_upper' Shift (2017-2019): `o_clean'", size(msmall)) xtitle("SDs from Nat. Mean (2017-2018 Avg)") ytitle("`c_title'") graphregion(color(white))
        capture graph export "`baseOut'/by_ownership/`o'/longdiff_2017_2019_`l'.png", replace width(2000)
        
        capture binscatter d_`l'_2023 bin_pre_2023 if year == 2023 & own_cat == "`o'", discrete line(connect) title("`l_upper' Shift (2020-2023): `o_clean'", size(msmall)) xtitle("SDs from Nat. Mean (2017-2020 Avg)") ytitle("`c_title'") graphregion(color(white))
