*===============================================================================
* SCRIPT: 08h_provider_inpatient_mips.do
* PURPOSE: Provider-Level Behavioral Benchmarks for MIPS (Inpatient)
* FEATURES: Ultra-Fast Engine, Dynamic Lags, Unified Legends, Multi-line Braces
*===============================================================================
set more off, permanently
include "C:/Users/omarf/Dropbox/personal_files_omar_farrag/Research/projects/NYS_npPolicy/code/00_initialize.do"

* --- START LOG ---
local script_name "08h_provider_inpatient_mips_test"
...
display "=== STARTING MIPS BENCHMARKS FOR: INPATIENT PROVIDERS ==="

* --- 1. DYNAMIC FOLDER CREATION ---
capture mkdir "$outRoot/summary_stats/in_patient/benchmarks_mips_provider"
local baseOut "$outRoot/summary_stats/in_patient/benchmarks_mips_provider"
capture mkdir "`baseOut'"

* --- 2. LOAD DATA & TEST SAMPLE ---
use "$dataRoot/master_provider_inpatient_2013_2023.dta", clear
capture drop if missing(mips_final_score)
capture drop if mips_final_score < 0
...
foreach geo in "nat" "state" "county" "zip" {
    foreach m in `m_list' {
        ...
        local v_name ""
        if "`m'" == "m_final" {
            local v_name "mips_final_score"
        }
        else if "`m'" == "m_qual" {
            local v_name "mips_quality_score"
        }
        else if "`m'" == "m_pi" {
            local v_name "mips_pi_score"
        }
        else if "`m'" == "m_ia" {
            local v_name "mips_ia_score"
        }
        else if "`m'" == "m_cost" {
            local v_name "mips_cost_score"
        }
...
display "=== PROVIDER INPATIENT MIPS BENCHMARKS COMPLETE ==="
