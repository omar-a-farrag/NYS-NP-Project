*===============================================================================
* SCRIPT: 09c_provider_hcahps_benchmarks.do
...
* --- START LOG ---
local script_name "09c_provider_hcahps_benchmarks_nys"
...
capture mkdir "$outRoot/summary_stats/in_patient/benchmarks_hcahps_provider_nys"
local baseOut "$outRoot/summary_stats/in_patient/benchmarks_hcahps_provider_nys"
...
* --- SPECIALTY GROUPING (2 Categories) ---
* Category 1 — General Medicine: $cond_gen_med (includes Family & General Practice)
* Category 2 — Other Specialties: all remaining CMS specialties not in group 1
gen spec_num = 2
replace spec_num = 1 if $cond_gen_med
label define sp_lbl 1 "General Medicine" 2 "Other Specialties"
...
foreach dim in `dim_list' {
    ...
    *                          Orthopedic Surgery, Radiology, Oncology, etc.)
    local dept_note ""
    if "`dim'" == "spec" {
        local dept_note "(1) General Medicine: Emerg. Medicine, Family Medicine/Practice, General Medicine/Practice, Hospitalist, Internal Medicine, NP, PA, Pain Management. (2) Other Specialties: e.g., Cardiology, Orthopedic Surgery, Radiology, Oncology."
    }
	...
	foreach nv of local nest_lvls {
            ...
            * RELOAD FRESH DATA ONCE PER NEST LEVEL
            use `master_clean', clear
            
            local n_title ""
            local final_path ""
            
            * PATH SHORTENING LOGIC: Skip redundant folders to avoid Windows MAX_PATH limits
            if "`nest'" == "none" {
                local final_path "`baseOut'/`dim_folder'"
                capture mkdir "`final_path'"
            }
            else {
                ...
                local final_path "`baseOut'/`dim_folder'/`nest_folder'/`n_clean'"
                    ...
                    * CHART 1: DECLINE (RAM PROTECTED)
                    ...
                    if _N > 0 {
                        ...
                        *capture graph export "`baseOut'/`dim_folder'/`nest_folder'/`n_clean'/decline_`m'_by_`var'.png", replace width(2000)
						display "Attempting to export: `dim' - `nest' - `var' - `m'"
                        graph export "`final_path'/decline_`m'_by_`var'.png", replace width(2000)
                    }
                    restore
                    
                    * CHART 2+: GEO BINS (RAM PROTECTED)
                    foreach geo in "nat" "state" "county" "zip" {
                        local gname ""
                        if "`geo'" == "nat" {
                            local gname "NYS"
                        ...
                        if _N > 0 {
                            ...
                            *capture graph export "`baseOut'/`dim_folder'/`nest_folder'/`n_clean'/bin_`m'_`geo'_by_`var'.png", replace width(2000)
							display "Attempting to export: `dim' - `nest' - `var' - `m'"
                            graph export "`final_path'/bin_`m'_`geo'_by_`var'.png", replace width(2000)
                        }
                        restore
                    } // End geo loop
                } // End m_list loop
                
                * Flush memory strictly inside the var loop after all 'm' charts are exported
                capture graph drop _all
                
            } // End behavior_vars loop
        } // End nest_lvls loop
    } // End nest_list loop
} // End dim_list loop
