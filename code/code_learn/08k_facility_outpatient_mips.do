*===============================================================================
* SCRIPT: 08k_facility_outpatient_mips.do
* PURPOSE: Facility-Level Benchmarks for MIPS (ASC Market Proxy Aggregates)
...
* --- START LOG ---
local script_name "08k_facility_outpatient_mips"
log using "$logRoot/`script_name'.smcl", replace
* -----------------

capture ssc install binscatter
display "=== STARTING MIPS BENCHMARKS FOR: OUTPATIENT FACILITIES (ASCS) ==="

* --- 1. EXTRACT DATA FROM PROVIDERS (ZIP-LEVEL MARKET PROXY) ---
display "1. Aggregating Provider Behaviors, Labor, & MIPS to Local Market (ZIP)..."
...
bysort asc_id: egen pre_m100_1719 = mean(cond(year>=2017 & year<=2018, mips_final_score, .))
bysort asc_id: egen pre_m100_2023 = mean(cond(year>=2017 & year<=2020, mips_final_score, .))

foreach p in "np" "md" "pa" {
    bysort asc_id: egen `p'_17 = max(cond(year==2017, pct_`p', .))
...
* --- 6. GRAPHING ENGINE ---
display "5. Generating Journal-Ready MIPS ASC Visualizations with Deltas..."
local baseOut "$outRoot/summary_stats/out_patient/benchmarks_mips_facility"
        ...
        * --- MIPS METRICS LOOP ---
        foreach m in `m_list' {
            if "`m'" == "m_final" {
                ...
                local m_var "m_final"
                local ft_meas "Metric: Local Market Average MIPS Final Payment Adjustment Score."
            }
            else if "`m'" == "m_qual" {
                local m_title "MIPS Quality Score"
                local m_var "m_qual"
                local ft_meas "Metric: Local Market Average MIPS Quality Performance Category."
            }
            else if "`m'" == "m_pi" {
                local m_title "Interoperability Score"
                local m_var "m_pi"
                local ft_meas "Metric: Local Market Average MIPS Promoting Interoperability Category."
            }
            else if "`m'" == "m_ia" {
                local m_title "Improvement Activities Score"
                local m_var "m_ia"
                local ft_meas "Metric: Local Market Average MIPS Improvement Activities Category."
            }
            else if "`m'" == "m_cost" {
                local m_title "MIPS Cost Score"
                local m_var "m_cost"
                local ft_meas "Metric: Local Market Average MIPS Cost Performance Category."
            }
            
            local ft_dec "Decline: Binary (1=Yes) if the market's average `m_title' dropped from two years prior to one year prior."
...
foreach l in `lab_vars' {
    ...
    capture binscatter d_`l'_1719 bin_pre_1719 if year == 2019, discrete line(connect) title("`l_upper' Shift (2017-2019) by Pre-Sample", size(msmall)) xtitle("SDs from Nat. Mean (2017-2018 Avg)") ytitle("`c_title'") note("Outcome: Absolute change in `l_upper' proportion (2017-2019)." "`ft_pre1'", size(vsmall)) graphregion(color(white))
    capture graph export "`baseOut'/universal/longdiff_2017_2019_`l'.png", replace width(2000)
    
    capture binscatter d_`l'_2023 bin_pre_2023 if year == 2023, discrete line(connect) title("`l_upper' Shift (2020-2023) by Pre-Sample", size(msmall)) xtitle("SDs from Nat. Mean (2017-2020 Avg)") ytitle("`c_title'") note("Outcome: Absolute change in `l_upper' proportion (2020-2023)." "`ft_pre2'", size(vsmall)) graphregion(color(white))
    ...
    foreach a in `auths' {
        ...
        capture binscatter d_`l'_1719 bin_pre_1719 if year == 2019 & np_auth == "`a'", discrete line(connect) title("`l_upper' Shift (2017-2019): `a_clean'", size(msmall)) xtitle("SDs from Nat. Mean (2017-2018 Avg)") ytitle("`c_title'") graphregion(color(white))
        capture graph export "`baseOut'/by_np_auth/`a'/longdiff_2017_2019_`l'.png", replace width(2000)
        
        capture binscatter d_`l'_2023 bin_pre_2023 if year == 2023 & np_auth == "`a'", discrete line(connect) title("`l_upper' Shift (2020-2023): `a_clean'", size(msmall)) xtitle("SDs from Nat. Mean (2017-2020 Avg)") ytitle("`c_title'") graphregion(color(white))
        capture graph export "`baseOut'/by_np_auth/`a'/longdiff_2020_2023_`l'.png", replace width(2000)
    }
    * MEMORY FIX
    capture graph drop _all
}
display "=== FACILITY OUTPATIENT MIPS BENCHMARKS COMPLETE ==="