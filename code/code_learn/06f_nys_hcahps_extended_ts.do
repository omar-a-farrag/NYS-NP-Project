*===============================================================================
* SCRIPT: 06f_nys_hcahps_extended_ts.do
* PURPOSE: 17-Year Time-Series Line Charts for HCAHPS (NYS Focus).
* FEATURES: NYS Comparative Benchmarks, NYS Subgroups (Own/Geo), No Scope Loops
*===============================================================================
set more off, permanently
include "C:/Users/omarf/Dropbox/personal_files_omar_farrag/Research/projects/NYS_npPolicy/code/00_initialize.do"

* --- START LOG ---
local script_name "06f_nys_hcahps_extended_ts"
log using "$logRoot/`script_name'.smcl", replace
* -----------------

display "=== STARTING NYS EXTENDED HCAHPS TIME SERIES (2007-2024) ==="
...
local baseDir "$outRoot/summary_stats/in_patient/time_series_nys/hcahps_extended"
capture mkdir "$outRoot/summary_stats/in_patient/time_series_nys"
capture mkdir "`baseDir'"

local subfolders "overall by_ownership by_geography"
...
local gap_note = ""
if r(N) == 0 {
    local gap_note "Gap in 2022 reflects data availability."
}
...
foreach var in `hcahps_vars' {
    ...
    * ---------------------------------------------------------
    * 1. Overall Comparative Benchmark (NYS vs Nat vs FPA)
    * ---------------------------------------------------------
    use `master_clean', clear
    capture drop if missing(`var')
    
    * Generate 3 Comparative Benchmarks before subsetting to NYS
    ...
    * ---------------------------------------------------------
    * 3. By Geography: Urban vs Rural (NYS ONLY)
    * ---------------------------------------------------------
    use `master_clean', clear
    keep if state_str == "NY"
    ...
    if r(N) > 0 {
        ...
        if _N > 0 {
            twoway (connected `var' year if is_urban==0, lcolor(gs8) lpattern(dash) msymbol(Oh)) ///
                ...
                title("NYS: `clean_title' by Geography", size(medium) color(black)) ///
                ytitle("Mean Value") xtitle("Year") xlabel(2007(1)2024, angle(45) labsize(vsmall)) ///
                legend(order(1 "Rural" 2 "Urban") position(6) rows(1)) ///
                note("`full_note'", position(7) justification(left) size(vsmall) margin(t=3 l=2) span) ///
                graphregion(color(white) `graph_margin')
            capture graph export "`baseDir'/by_geography/ts_geo_`var'.png", replace width(2000)
        }
    }
}
display "=== NYS EXTENDED HCAHPS TIME SERIES COMPLETE ==="