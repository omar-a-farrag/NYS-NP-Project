...
* --- 6. GRAPHING ENGINE ---
display "5. Generating Journal-Ready ASC Visualizations with Deltas..."
local baseOut "$outRoot/summary_stats/out_patient/benchmarks_oas_facility"
...
foreach l in `lab_vars' {
    ...
    local ft_pre1 "Benchmark: SD from National mean during pre-period (2015-2016)."
    local ft_pre2 "Benchmark: SD from National mean during pre-period (2015-2020)."

    capture binscatter d_`l'_1619 bin_pre_1619 if year == 2019, discrete line(connect) title("`l_upper' Shift (2016-2019) by Pre-Sample", size(msmall)) xtitle("SDs from Nat. Mean (2015-2016 Avg)") ytitle("`c_title'") note("Outcome: Absolute change in `l_upper' proportion (2016-2019)." "`ft_pre1'", size(vsmall)) graphregion(color(white))
    capture graph export "`baseOut'/universal/longdiff_2016_2019_`l'.png", replace width(2000)
    
    capture binscatter d_`l'_2023 bin_pre_2023 if year == 2023, discrete line(connect) title("`l_upper' Shift (2020-2023) by Pre-Sample", size(msmall)) xtitle("SDs from Nat. Mean (2015-2020 Avg)") ytitle("`c_title'") note("Outcome: Absolute change in `l_upper' proportion (2020-2023)." "`ft_pre2'", size(vsmall)) graphregion(color(white))
    ...
    foreach a in `auths' {
        ...
        capture binscatter d_`l'_1619 bin_pre_1619 if year == 2019 & np_auth == "`a'", discrete line(connect) title("`l_upper' Shift (2016-2019): `a_clean'", size(msmall)) xtitle("SDs from Nat. Mean (2015-2016 Avg)") ytitle("`c_title'") graphregion(color(white))
        capture graph export "`baseOut'/by_np_auth/`a'/longdiff_2016_2019_`l'.png", replace width(2000)
        
        capture binscatter d_`l'_2023 bin_pre_2023 if year == 2023 & np_auth == "`a'", discrete line(connect) title("`l_upper' Shift (2020-2023): `a_clean'", size(msmall)) xtitle("SDs from Nat. Mean (2015-2020 Avg)") ytitle("`c_title'") graphregion(color(white))
