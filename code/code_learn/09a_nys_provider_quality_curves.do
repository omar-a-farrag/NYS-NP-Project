*===============================================================================
* SCRIPT: 08a_nys_provider_quality_curves.do
* PURPOSE: Binscatters mapping Provider Behavior vs Provider MIPS - NEW YORK STATE ONLY
...
* --- START LOG ---
local script_name "09a_nys_provider_quality_curves"
...
foreach setting in "in_patient" "out_patient" {
    display "=== STARTING NYS QUALITY CURVES FOR: `setting' PROVIDERS ==="
    ...
    * --- 1. DYNAMIC FOLDER CREATION ---
    ...
    capture mkdir "$outRoot/summary_stats/`setting'/quality_curves/provider_analysis_nys"
    local baseDir "$outRoot/summary_stats/`setting'/quality_curves/provider_analysis_nys"
    ...
    * --- 2. LOAD, FILTER TO NYS & SAMPLE DATA ---
    ...
    * --- 4. PRE-GENERATE LAGS, 2-YEAR LAGS & LEADS (xtset Safe) ---
            ...
            foreach d in "quality_predicts_behavior" "behavior_predicts_quality" {
                foreach t in "current" "lag" "lead" "lag2" {
                    ...
                    * --- Initialize locals to prevent stale values ---
                    ...
                    if "`d'" == "quality_predicts_behavior" {
                        ...
                        if "`t'" == "current" {
                            ...
                            local t_note "Timing: Both variables represent concurrent year performance."
                        }
                        else if "`t'" == "lag" {
                            local x_var "lag_`q_var'"
                            local x_label "Prior Year `q_title'"
                            local t_dir "lag_prior_year"
                            local t_note "Timing: X-Axis (Quality) is lagged. Reflects PRIOR year performance."
                        }
                        else if "`t'" == "lead" {
                            local x_var "lead_`q_var'"
                            local x_label "Next Year `q_title'"
                            local t_dir "lead_next_year"
                            local t_note "Timing: X-Axis (Quality) is leading. Reflects NEXT year performance."
                        }
                        else if "`t'" == "lag2" {
                        ...
                        if "`t'" == "current" {
                            ...
                            local t_dir "lag_prior_year"
                            local t_note "Timing: X-Axis (Behavior) is lagged. Reflects PRIOR year performance."
                        }
                        else if "`t'" == "lead" {
                            local x_var "lead_`b_var'"
                            local x_label "Next Year `b_title'"
                            local t_dir "lead_next_year"
                            local t_note "Timing: X-Axis (Behavior) is leading. Reflects NEXT year performance."
                        }
                        else if "`t'" == "lag2" {
                    ...
                    * --- Guard: skip if locals not set ---
                    ...
                    if r(N) > 0 {
                        capture binscatter `y_var' `x_var', by(prov_type) title("NYS By Prov: `y_label' vs `x_label'", size(medium) color(black)) legend(order(1 "MD/DO" 2 "NP" 3 "PA") position(6) rows(1)) `common_opts'
                    ...
                    if r(N) > 0 {
                        capture binscatter `y_var' `x_var', by(female_num) title("NYS By Gender: `y_label' vs `x_label'", size(medium) color(black)) legend(order(1 "Male" 2 "Female") position(6) rows(1)) `common_opts'
                    ...
                    if r(N) > 0 {
                        ...
                        capture binscatter `y_var' `x_var', by(dept_cat) title("NYS By Dept: `y_label' vs `x_label'", size(medium) color(black)) legend(order(1 "Specialty" 2 "Gen Med" 3 "Primary Care") position(6) rows(1)) `d_opts'
                    ...
                    if r(N) > 0 {
                        capture binscatter `y_var' `x_var', by(grad_decade) title("NYS By Grad Decade: `y_label' vs `x_label'", size(medium) color(black)) legend(order(1 "Pre-1990" 2 "1990s" 3 "2000s" 4 "2010s+") position(6) rows(1)) `common_opts'
...
display "=== NYS PROVIDER QUALITY CURVES COMPLETE ==="
